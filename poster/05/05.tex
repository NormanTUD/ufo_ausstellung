...
\newcommand{\plotPrimeCount}[1]{%
    \typeout{Versuche, die Datei 'prime_counts.csv' zu laden...} % Debug-Meldung
    \pgfplotstablenew[
        col sep=comma,
        x=x,
        y=pi % Der Spaltenname 'pi' muss genau so stimmen
    ]{prime_counts.csv}\loadedtable
    \typeout{prime_counts.csv geladen}

    % Debug-Meldung, um den Inhalt der geladenen Tabelle anzuzeigen
    \pgfplotstabletypeset{\loadedtable} % Gibt die geladene Tabelle aus, was bei Problemen helfen kann.

    % Überprüfe, ob die Tabelle erfolgreich geladen wurde
    \ifnum\pgfplotstable@count=0
        \typeout{Warnung: Die Tabelle ist leer!}
    \else
        \typeout{Die Tabelle wurde erfolgreich geladen.} % Debug-Meldung für Erfolg
    \fi
    ...
    \begin{figure}[h]
        ...
        \begin{tikzpicture}
            \begin{axis}[
                ...
                domain=0:#1,
                samples=100,
                restrict y to domain=0:1000 % Passen Sie dies an, um den Y-Bereich zu beschränken
            ]
            \addplot[only marks, color=blue] table [x=x, y=pi] {\loadedtable}; % Stelle sicher, dass hier y=pi ist
            \end{axis}
        \end{tikzpicture}
    \end{figure}
}
