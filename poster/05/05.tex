...
\newcommand{\plotPrimeCount}[1]{%
    ...
    \pgfplotstablenew[
        col sep=comma,
        x=x,
        y=pi % Achte darauf, dass der Spaltenname hier korrekt ist
    ]{prime_counts.csv}\loadedtable

    % Debug-Meldung, ob die Tabelle erfolgreich geladen wurde
    \pgfplotstabletypeset{\loadedtable} % Gibt eine Fehlermeldung aus, wenn die Tabelle nicht geladen werden kann.
    \typeout{Die Tabelle wurde erfolgreich geladen.} % Debug-Meldung
    ...
    \begin{figure}[h]
        ...
        \begin{tikzpicture}
            ...
            \addplot[only marks, color=blue] table [x=x, y=pi] {\loadedtable}; % Korrektur von y=pix zu y=pi
            \end{axis}
        \end{tikzpicture}
    \end{figure}
}
